...
test.describe('File Operations', () => {
  ...
  test('should display file grid view by default', async ({ page }) => {
    // Wait for loading to finish and at least one file card or empty state to render
    await expect(page.locator('[class*="file-card"], [class*="file-row"], [data-testid="empty-state"]').first()).toBeVisible({ timeout: 10000 });
  });
